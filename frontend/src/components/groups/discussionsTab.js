...
const DiscussionTab = ({groupId}) => {
    ...
    const [postsPerPage] = useState(3); // Liczba postów na stronę
    ...
    const fetchGroupData = async () => {
        try {
            const response = await client.get(API_BASE_URL + `forum/${groupId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
                                    ...
                                    {post.comments?.slice(0, 3).map(comment => (
                                        <Box key={comment.id} sx={{mb: 2}}>
                                            <Box sx={{display: 'flex', gap: 1}}>
                                                <Avatar src={comment.author.profile_picture?.slice(47)}
